...
function parseData(flightsData: Root) {
  const flights = [...flightsData.best_flights, ...flightsData.other_flights];
  console.log(flights)
}

async function loadTestData() {
  const filePath = path.join(process.cwd(), "play.json");
  const fileContents = await fs.readFile(filePath, "utf-8");
  const flights = await JSON.parse(fileContents);
